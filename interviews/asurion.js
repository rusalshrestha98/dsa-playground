...
function transform(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (arr[i] == arr[-1]) {
      // if yes, don't grab subsequent elements
      return;
    } else {
      // if no, multiple all subsequent elements with current index and push into result array
      let currElement = arr[i]; // set current element
      const subElements = arr.slice(i + 1); // grab subsequent elements
      subElements.map((d) => {
        // multiply curreent element with subsequent elements
...
//     subElements.map((x) => (currElement *= x));
//     result.push(currElement);
//   }

//   return result; // [240, 80, 40, 20, 5]
// };
